function en_to_bn_number_conversion(en_number) {
	...
	for (var i = 0; i < en_number.length; i++) {
		if (en_number[i] == '0') bn_number = bn_number + "۰";
		if (en_number[i] == '1') bn_number = bn_number + "۱";
		if (en_number[i] == '2') bn_number = bn_number + "۲";
		if (en_number[i] == '3') bn_number = bn_number + "۳";
		if (en_number[i] == '4') bn_number = bn_number + "۴";
		if (en_number[i] == '5') bn_number = bn_number + "۵";
		if (en_number[i] == '6') bn_number = bn_number + "۶";
		if (en_number[i] == '7') bn_number = bn_number + "۷";
		if (en_number[i] == '8') bn_number = bn_number + "۸";
		if (en_number[i] == '9') bn_number = bn_number + "۹";
...
function showPrayTimes(lat, long, prayTimes) {
	...
	var listBN = ['سهار', 'لمر پريوتو وقت', 'ماسپښین', 'مازدیګر', 'Sunset', 'ماښام', 'ماخستن', 'نیمه شپه'];
	...
	if($("input[name='lang']:checked").val() === 'bn'){
		var table_footer_title = "د بل لمانځه وخت پاتې شو"; //পরবর্তী নামাজের সময় বাকী
	}
...
function setCurrentDateTime() {
	if($("input[name='lang']:checked").val() === 'bn'){
		moment.locale('bn');
		document.getElementById("table_header_title").innerHTML = "د لمانځه وخت"; //নামাজের সময়
		document.getElementById("footer_note").innerHTML = "* روښانه قطارونه اوسنی وخت څرګندوي"; //* উজ্জ্বল সারি বর্তমান ওয়াক্ত প্রকাশ করে
	}else{
		moment.locale('en');
